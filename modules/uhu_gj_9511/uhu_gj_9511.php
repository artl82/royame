...
class Uhu_gj_9511 extends Module
{

	public function __construct()
	{
		$this->name = 'uhu_gj_9511';
		$this->tab = 'others';
		...
		$this->displayName = 'uhu Advertising block 9511';
		$this->description = $this->l('Adds a block to display one advertisement.');
	}
	...
	public function hookHome($params)
	{
		...
		$this->smarty->assign(array(
			...
			'zoom' => Configuration::get('uhu_modvalue_'.$mod_name.'_6')
		));
		...
		$advimgs = explode('|', Configuration::get('uhu_modvalue_'.$mod_name.'_'.$adv_image_0));
		if (isset($advimgs[$lang_id]))
			$adv = $advimgs[$lang_id];
		else
			$adv = $advimgs[0];
		if (strstr($adv, 'http://') <> '')
			$this->smarty->assign('adv_image_0', $adv);
		else
			$this->smarty->assign('adv_image_0', $imgurl.$adv);
		...
		{
			if (isset($_COOKIE['gj9511']))
			{
				if($_COOKIE['gj9511'] == 'show')
